...
### 🎞️ Exibição na PDP e uso em listagens

O atributo foi configurado com:
...
Essas configurações foram mantidas porque o requisito atual utiliza o selo apenas na página individual do produto.

`visible_on_front = false` evita que o Magento exiba automaticamente o atributo através dos blocos padrão de atributos do produto. A exibição do selo é controlada pelo próprio módulo através de `sustainable-seal.phtml`.

`used_in_product_listing = false` indica que o atributo não precisa ser carregado nas collections utilizadas nas listagens de catálogo, já que atualmente ele não é utilizado em páginas de categoria ou grids de produtos da storefront.

Caso o selo passe a ser exibido futuramente em listagens de categoria, essa configuração deverá ser alterada por código e os índices correspondentes deverão ser atualizados.
